...
const MONITORING_TERMS = /\b(soil\s+moisture|moisture|kelembapan(?:\s+tanah)?|kondisi\s+tanah|pH(?:\s+tanah)?|suhu|sensor(?:\s+(?:pertanian|tanah|kelembapan|moisture))?|IoT(?:\s+(?:pertanian|tanaman))?|data\s+(?:monitoring|sensor|historis)|monitoring\s+(?:tanaman|nursery)|bedengan|penyiraman|disiram|kebutuhan\s+air|curah\s+hujan|rainfall|cuaca)\b/i;
