...
alter table public.rainfall_readings
  ...
  add column if not exists bedengan text,
  add column if not exists rainfall_mm numeric,
  add column if not exists rainfall_value numeric,
...
begin
  if not exists (
    select 1
    from pg_constraint
    where conname = 'rainfall_readings_rainfall_value_check'
  ) then
    alter table public.rainfall_readings
      add constraint rainfall_readings_rainfall_value_check
      check (rainfall_value >= 0) not valid;
  end if;

  if not exists (
    select 1
    from pg_constraint
    where conname = 'rainfall_readings_source_check'
  ) then
    alter table public.rainfall_readings
      add constraint rainfall_readings_source_check
      check (source in ('ombrometer', 'manual')) not valid;
  end if;

  if not exists (
    select 1
    from pg_constraint
    where conname = 'rainfall_readings_unit_check'
  ) then
    alter table public.rainfall_readings
...
alter table public.rainfall_readings
  drop constraint if exists rainfall_readings_source_check;
...
alter table public.rainfall_readings validate constraint rainfall_readings_rainfall_value_check;
alter table public.rainfall_readings validate constraint rainfall_readings_source_check;
alter table public.rainfall_readings validate constraint rainfall_readings_unit_check;

alter table public.rainfall_readings
  alter column rainfall_value set not null,
...
create index if not exists rainfall_readings_created_at_idx
  on public.rainfall_readings (created_at desc);

drop trigger if exists set_rainfall_readings_updated_at on public.rainfall_readings;
create trigger set_rainfall_readings_updated_at
before update on public.rainfall_readings
for each row
execute function public.set_updated_at();

alter table public.rainfall_readings enable row level security;

drop policy if exists "rainfall_readings_read_all" on public.rainfall_readings;
create policy "rainfall_readings_read_all"
on public.rainfall_readings
for select
to anon, authenticated
using (true);

drop policy if exists "rainfall_readings_insert_all" on public.rainfall_readings;
create policy "rainfall_readings_insert_all"
on public.rainfall_readings
for insert
to anon, authenticated
with check (true);
