...
const NurserySummaryCards = ({ summary }) => {
  const cards = [
    ...
    { label: "Soil pH Global", value: summary?.soilPh == null ? "-" : Number(summary.soilPh).toFixed(2), detail: summary?.soilPhStatus === "active" ? "Aktif" : summary?.soilPhStatus === "inactive" ? `Tidak Aktif · terakhir ${new Intl.DateTimeFormat("id-ID", { hour: "2-digit", minute: "2-digit" }).format(new Date(summary.soilPhMeasuredAt))}` : "Belum ada data", icon: FiActivity, color: "text-emerald-700", bg: "bg-emerald-50" },
  ];

  return <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-5">{cards.map(({ label, value, detail, icon: Icon, color, bg }) => <div key={label} className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm"><div className="flex items-start justify-between gap-3"><div><p className="text-sm font-semibold text-slate-600">{label}</p><p className="mt-3 text-2xl font-bold tracking-tight text-slate-900">{value ?? "-"}</p><p className="mt-1 text-xs text-slate-500">{detail}</p></div><span className={`flex h-10 w-10 items-center justify-center rounded-md ${bg} ${color}`}><Icon className="text-xl" aria-hidden="true" /></span></div></div>)}</div>;
};
