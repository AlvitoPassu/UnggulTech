      ...
      {isOpen && (
        <section className="mb-3 flex h-[min(680px,calc(100dvh-120px))] w-[calc(100vw-2.5rem)] max-w-[400px] flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-xl shadow-slate-900/10" aria-label="Unggul AI Assistant">
          ...
          <form onSubmit={(event) => { event.preventDefault(); sendMessage(); }} className="border-t border-slate-100 p-3">
            <div className="flex items-center gap-2 rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 focus-within:border-[#1DAADF] focus-within:ring-2 focus-within:ring-[#1DAADF]/15"><textarea id="unggul-chat-input" value={input} onChange={(event) => setInput(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter" && !event.shiftKey) { event.preventDefault(); sendMessage(); } }} disabled={isLoading} rows="1" maxLength="2000" placeholder="Ketik pertanyaan..." className="max-h-24 min-h-5 flex-1 resize-none self-center bg-transparent text-sm leading-5 text-slate-700 outline-none placeholder:text-slate-400 disabled:cursor-not-allowed" /><button type="submit" disabled={!input.trim() || isLoading} className="rounded-lg bg-[#1DAADF] p-2 text-white transition hover:bg-[#1686b3] disabled:cursor-not-allowed disabled:opacity-50" aria-label="Kirim pesan"><FiSend /></button></div>
