  ...
  return <div className="min-h-screen bg-[#F5F6F8] text-slate-800">
      ...
      <div className="mx-auto flex h-[73px] max-w-[1440px] items-center justify-between gap-3 px-4 sm:px-8">
        ...
        <div className="ml-auto flex shrink-0 items-center gap-3 sm:gap-5">
          <div className="flex items-center gap-2 text-right">
            <div className="hidden md:block"><p className="text-[11px] font-medium uppercase tracking-wide text-white/70">Terakhir diperbarui</p><p className="text-xs font-semibold text-white">{formatWita(new Date(), { hour: "2-digit", minute: "2-digit" })} WITA</p></div>
            <button type="button" onClick={() => window.location.reload()} className="rounded-md p-2 text-white transition hover:bg-white/15" aria-label="Refresh data"><FiRefreshCw aria-hidden="true" /></button>
          </div>
          <HeaderAuthStatus />
        </div>
      </div>
    ...
    {editingRainfall && <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/45 p-4" role="dialog" aria-modal="true" aria-labelledby="edit-rainfall-title">
      <form onSubmit={handleRainfallUpdate} className="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-lg bg-white shadow-xl">
        ...
        <div className="grid gap-4 p-5 sm:grid-cols-2"><label className="text-sm font-medium text-slate-700">Nilai Curah Hujan (mm)<input required min="0" step="any" type="number" value={editForm.rainfall_value} onChange={(event) => setEditForm((current) => ({ ...current, rainfall_value: event.target.value }))} className="mt-2 w-full rounded-md border border-slate-300 px-3 py-2 font-normal outline-none focus:border-[#1DAADF] focus:ring-2 focus:ring-[#d1f0fa]" /></label><label className="text-sm font-medium text-slate-700">Tanggal/Waktu Pengukuran<input required type="datetime-local" value={editForm.measured_at} onChange={(event) => setEditForm((current) => ({ ...current, measured_at: event.target.value }))} className="mt-2 w-full rounded-md border border-slate-300 px-3 py-2 font-normal outline-none focus:border-[#1DAADF] focus:ring-2 focus:ring-[#d1f0fa]" /></label><label className="text-sm font-medium text-slate-700">Nursery<select value={editForm.nursery} onChange={(event) => setEditForm((current) => ({ ...current, nursery: event.target.value, bedengan: "" }))} className="mt-2 w-full rounded-md border border-slate-300 bg-white px-3 py-2 font-normal outline-none focus:border-[#1DAADF] focus:ring-2 focus:ring-[#d1f0fa]"><option value="">Semua nursery</option>{rainfallNurseries.map((nursery) => <option key={nursery} value={nursery}>{nursery}</option>)}</select></label><label className="text-sm font-medium text-slate-700">Bedengan<select value={editForm.bedengan} onChange={(event) => setEditForm((current) => ({ ...current, bedengan: event.target.value }))} className="mt-2 w-full rounded-md border border-slate-300 bg-white px-3 py-2 font-normal outline-none focus:border-[#1DAADF] focus:ring-2 focus:ring-[#d1f0fa]"><option value="">Semua bedengan</option>{rainfallBedengans.map((bedengan) => <option key={bedengan} value={bedengan}>Bedengan {bedengan}</option>)}</select></label><label className="text-sm font-medium text-slate-700 sm:col-span-2">Catatan<textarea value={editForm.notes} onChange={(event) => setEditForm((current) => ({ ...current, notes: event.target.value }))} rows="3" className="mt-2 w-full rounded-md border border-slate-300 px-3 py-2 font-normal outline-none focus:border-[#1DAADF] focus:ring-2 focus:ring-[#d1f0fa]" /></label>{editError && <p className="rounded-md bg-red-50 px-3 py-2 text-sm text-red-700 sm:col-span-2">{editError}</p>}</div>
